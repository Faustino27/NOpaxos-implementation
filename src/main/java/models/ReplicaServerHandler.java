...
public class ReplicaServerHandler extends SimpleChannelInboundHandler<List<Packet>> {
    ...
    private void validateSequence(Packet packet) {
        ...
        long startTimeSign = System.nanoTime();
        ...
        if (!validSignature && packet.getHeader().getMessageType() != 4) {
            logger.info("Invalid signature. Dropping packet.");
            return;
        }
        if (packet.getHeader().getSequenceNumber() <= 10000) {
            timingResults.add(System.nanoTime() - startTimeSign);
        }
    ...
    private void sendReplicasOrderRequest(int sequenceNumber, Packet recivedPakcet) {
        ...
        for (Map.Entry<String, Channel> entry : replicaChannels.entrySet()) {
            ...
            if (replicaChannel != null && replicaChannel.isActive()) {
                // logger.info("Sending request to replica: " + entry.getKey() + " for missing
                // packets");
    ...
    private void writeTotalRequestsToFile() {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter("replica_info.txt"))) {
            writer.write("total requests made: " + replica.processados.get());
            writer.newLine();
            writer.write("total exectuion time: " + (System.nanoTime() - startTime));
            writer.newLine();
            writer.write("Time to verify all messages: " + (System.nanoTime() - startTime));
            ...
            logger.info(
                    "Writen total requests made to replica" + replica.getReplicaId()
